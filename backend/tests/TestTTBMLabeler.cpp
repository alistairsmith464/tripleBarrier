...
TEST(TTBMLabelerTest, ExponentialDecayBasic) {
    ...
    EXPECT_EQ(result[0].time_elapsed_ratio, 0.25);  // 1 period out of 4
...
TEST(TTBMLabelerTest, LinearDecayBasic) {
    ...
    EXPECT_EQ(result[0].time_elapsed_ratio, 0.5);  // 2 periods out of 4
...
TEST(TTBMLabelerTest, HyperbolicDecayBasic) {
    ...
    EXPECT_EQ(result[0].label, +1);  // Hard label should be +1
    EXPECT_GT(result[0].ttbm_label, 0.0);  // TTBM label should be positive
    EXPECT_EQ(result[0].time_elapsed_ratio, 0.75);  // 3 periods out of 4
...
TEST(TTBMLabelerTest, VerticalBarrierHit) {
    ...
    EXPECT_EQ(result[0].time_elapsed_ratio, 1.0);  // Full vertical barrier period
...
TEST(TTBMLabelerTest, InstantBarrierHit) {
    ...
    EXPECT_EQ(result[0].time_elapsed_ratio, 0.5);  // 1 period out of 2
