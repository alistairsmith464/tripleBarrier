...
TEST(FeatureExtractorTest, RollingStd5D_Basic) {
    vector<PreprocessedRow> rows;
    for (int i = 0; i < 6; ++i) rows.push_back(makeRow(100 + i * 2, "2021-01-" + to_string(i+1)));
    ...
    auto result = FeatureExtractor::extractFeaturesForClassification(features, rows, events);
    // With prices: 100, 102, 104, 106, 108, 110, returns are all 0.02, so std = 0
    EXPECT_NEAR(result.features[0][FeatureCalculator::ROLLING_STD_5D], 0.0, 0.01);
}

TEST(FeatureExtractorTest, RollingStd5D_Constant) {
    vector<PreprocessedRow> rows;
    for (int i = 0; i < 6; ++i) rows.push_back(makeRow(100, "2021-01-" + to_string(i+1)));
...
TEST(FeatureExtractorTest, RollingStd5D_ShortWindow) {
    vector<PreprocessedRow> rows;
    for (int i = 0; i < 6; ++i) rows.push_back(makeRow(10 * (i+1), "2021-01-" + to_string(i+1)));
    vector<LabeledEvent> events = {makeEvent(1, "2021-01-06", 60, 60)};
    set<string> features = {"Rolling standard deviation of daily returns over the last 5 days"};
    auto result = FeatureExtractor::extractFeaturesForClassification(features, rows, events);
    // Prices: 10, 20, 30, 40, 50, 60. Returns: all 1.0, std = 0
    EXPECT_NEAR(result.features[0][FeatureCalculator::ROLLING_STD_5D], 0.0, 0.01);
}
...
TEST(FeatureExtractorTest, Roc5D_Basic) {
    ...
    EXPECT_DOUBLE_EQ(result.features[0][FeatureCalculator::ROC_5D], 0.08);
}
...
TEST(FeatureExtractorTest, DaysSinceLastEvent_Basic) {
    ...
    EXPECT_EQ(result.features[0][FeatureCalculator::DAYS_SINCE_LAST_EVENT], 0);
}
...
TEST(FeatureExtractorTest, DaysSinceLastEvent_MultipleEvents) {
    ...
    EXPECT_EQ(result.features[1][FeatureCalculator::DAYS_SINCE_LAST_EVENT], 3);
    // For event at index 9: previous event at 5, so 4 days
    EXPECT_EQ(result.features[2][FeatureCalculator::DAYS_SINCE_LAST_EVENT], 4);
}
...
TEST(FeatureExtractorTest, DaysSinceLastEvent_FarApart) {
    ...
    EXPECT_EQ(result.features[1][FeatureCalculator::DAYS_SINCE_LAST_EVENT], 10);
    // For event at index 19: 9 days
    EXPECT_EQ(result.features[2][FeatureCalculator::DAYS_SINCE_LAST_EVENT], 9);
}
...
TEST(FeatureExtractorTest, DaysSinceLastEvent_StartAndEnd) {
    ...
    EXPECT_EQ(result.features[0][FeatureCalculator::DAYS_SINCE_LAST_EVENT], 0);
    // Last event: 6 days
    EXPECT_EQ(result.features[1][FeatureCalculator::DAYS_SINCE_LAST_EVENT], 6);
}
